...
describe("1.0 changelog contract", () => {
  it("announces usage and reliability in 1.0.3", () => {
    expect(CHANGELOG[0]).toMatchObject({ version: "1.0.3", titleKey: "whatsNew.r1003.title" });
  ...
  it("announces the Linux 1.0.2 patch", () => {
    ...
    expect(CHANGELOG[1]?.presentation).toBeUndefined();
    expect(CHANGELOG[1]?.highlights).toHaveLength(3);
  });
  ...
  it("announces the 1.0 milestone and adds the 1.0.1 fixes on the same screen", () => {
    ...
    expect(CHANGELOG[2]?.highlights).toHaveLength(5);
    expect(CHANGELOG[2]?.highlights.at(-1)).toMatchObject({
    ...
    expect(CHANGELOG[3]?.highlights).toHaveLength(4);
  });
