...
## In-app application (metacodex)

The spec above is the Cursor marketing system. metacodex is a dense desktop IDE. Translate the system; do not paste marketing sizes, 80px section rhythm, or orange Commit buttons into chrome.

### Themes

Two palettes only. Ids stay `porcelain` / `graphite` (settings migration).

| Role | Light (Porcelain) | Dark (Graphite) |
|---|---|---|
| canvas | `#f7f7f4` | `#141412` |
| canvas-soft | `#fafaf7` | `#1a1a17` |
| surface-card | `#ffffff` | `#1e1e1b` |
| surface-strong | `#e6e5e0` | `#2a2a26` |
| ink | `#26251e` | `#f7f7f4` |
| body | `#3f3d37` | `#d4d1c8` |
| muted | `#5c5a54` | `#a09c92` |
| hairline | `#e6e5e0` | `#2e2e28` |
| primary / accent | `#f54e00` | `#f54e00` |
| primary-active | `#d04200` | `#ff6a2a` |
| on-primary | `#ffffff` | `#ffffff` |

Dark is a warm inverse of the cream pair, not a cool slate. Mode in Settings is System (default) / Light / Dark.

### Type and font

Keep the existing IDE scale (`text-micro` 10 through `text-display-*`). Do not lift chrome to 16 / 18 / 72px. Font: `-apple-system` / SF Pro on Mac, Inter fallback, JetBrains Mono on every code and terminal surface. CursorGothic is licensed; do not add it.

### Color roles

- `--primary` and `--accent` are Cursor Orange. Scarce: selection rings, active identity bar, explorer recent tint, rare brand CTAs (error recovery).
- Chrome filled buttons follow `button-download`: `--ink` fill, `--canvas` text, `--ink-hover` on hover. Never `text-on-primary` on an ink fill (`on-primary` is white for orange; dark ink is cream).
- `text-on-primary` stays on orange, danger, and warn fills only.
- Timeline pastels (`--timeline-*`) exist as CSS tokens. In-product action stages only, never chrome or system actions.
- Depth is hairline-only. `--shadow-elevated` is a 1px hairline. Drag ghost may keep a quiet shadow.
- Atmosphere is a warm ink (light) or cream (dark) whisper, never a lavender glow.

### Anti-patterns (in-app)

- Do not paint Commit, Confirm, or segmented controls orange.
- Do not resurrect Tokyo Night, Solar Cream, or other gallery palettes.
- Do not mix cool slate surfaces with this cream pair.
- Do not use timeline pastels as button or status-dot colors.
