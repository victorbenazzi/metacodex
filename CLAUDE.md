...
- **Explorer mutations (partial):** inline create file/folder exists (roots-checked Rust commands, atomic writes). Rename / delete / drag-move in the tree are NOT implemented yet (known VS Code parity gap). `tabsStore.remapForRename` reacts to `fs://renamed` (today emitted only by preview "send to project"); `closeForRemovedPath` reacts to deletions detected via the watcher + stat.
- **Icons:** Hugeicons stroke-rounded (free set) via the central registry `src/components/ui/icons.tsx`; never import `@hugeicons/*` directly in components, and never resolve icons by dynamic name lookup (kills tree-shaking). Project glyphs resolve through `projectIdentity.ts::lookupProjectGlyph`; picker names in `PROJECT_ICONS` are persisted, so never rename/remove entries.
