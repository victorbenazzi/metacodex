...
- **Entidade**: cada Agente vive em `~/.metacodex/agents/<slug>/` (repo git; o harness commita checkpoints): `AGENT.md` (persona), `agent.json` (modelo, preset, projetos, heartbeat, dreamAfterRuns, continuationCap, avatar), `HEARTBEAT.md`, `MEMORY.md` + `memory/` (camadas global e `memory/projects/<key>/`), `skills/`, `reports/`, `logs/` (`runs.jsonl` + `state.json`), `journal/`, `proposals/`. Rust: `agent/entities.rs` (CRUD, slug = boundary de segurança via `home_dir`) + `agent/life.rs` (memória, logs, reports, propostas, prompts de heartbeat/dream).
...
- **Chat**: AgentPicker no composer espelha a entidade selecionada em `chat.store.entity` (deps continuam periferia → chat.store); entity trava modelo/variant/preset (pickers individuais somem) e troca de entidade inicia sessão nova. Memória entra como `system` em todo send (`chat.store::entitySystem` → `agent_entity_memory_context`).
- **Execução autônoma**: `scheduler.rs::run_entity_execution` (future BOXADA: execução spawna dream que é execução) + `runtime.rs::run_entity_turn`. Preset DO agente (espelho Rust dos 3 rulesets em `ruleset_for_preset`), Continuação `CONTINUE:` / `CONTINUE_IN N:` com cap, report escrito pelo harness, dream após N execuções (full-auto, `?directory=<home>`), heartbeat no tick do scheduler (`HEARTBEAT_OK` = log-only). Permissão pendente em run autônoma: notificação OS + abort no budget de 30 min; aprovar = abrir a conversa da run na sidebar.
...
| Execução autônoma de agente (continuação, report, dream, heartbeat) | `src-tauri/src/agent/scheduler.rs::run_entity_execution` + `runtime.rs::run_entity_turn` + `life.rs` (prompts e arquivos) |
