...
- **Floating placement (menus, popovers, flyouts):** every floating element keeps an 8px margin from the viewport edge and opens AWAY from the nearest screen edge (a trigger on the left opens to the right, and vice-versa); pick `align`/`side` by the trigger's nearest edge, never a fixed value. Prefer a Radix primitive: `DropdownMenu`/`ContextMenu` bring collision flip/shift AND a submenu "safe triangle" (pointer grace area) for free, and the shared wrappers in `components/ui/` default to `collisionPadding={8}` (so do NOT re-add it per call site). `Select`/`Tooltip` carry the same default. For a CUSTOM panel that can't be a Radix menu (it must keep input focus, e.g. `MentionPopup`, `AvatarPicker`), position it with `useAnchoredPopup` (`components/ui/useAnchoredPopup.ts`, Floating UI flip/shift/size; absolute strategy + no portal by default); add `safePolygon` only for a hover-driven cascading flyout. See `MENU_UX_PLAN.md`.
